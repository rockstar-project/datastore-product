...
ACCESS_TOKEN=$(curl --silent -X POST -H 'Accept: application/json' -H 'Content-Type: application/json; charset=UTF-8' -d '{"username":"'"$ADMIN_USER"'", "password":"'"$ADMIN_PASSWORD"'", "client_id":"'"$ADMIN_CLIENT_ID"'", "client_secret":"'"$ADMIN_CLIENT_SECRET"'", "audience":"https://api.rock-star.io/", "grant_type":"password" }' https://rockstar.auth0.com/oauth/token | jq -r .access_token)

RESPONSE_HEADER=$(curl -i -s -X POST $ADMIN_ENDPOINT_URL/products --data @product.json --header "Accept:application/json" --header "Content-Type:application/json; charset=UTF-8" --header "Authorization: Bearer $ACCESS_TOKEN")
